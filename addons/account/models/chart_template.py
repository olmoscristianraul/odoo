...
class AccountChartTemplate(models.Model):
    ...
    def _load(self, sale_tax_rate, purchase_tax_rate, company):
        ...
        if company.account_sale_tax_id:
            self.env['account.account'].browse(acc_template_ref.values()).filtered(lambda x: x.user_type_id.internal_group == 'income').tax_ids = company.account_sale_tax_id
        if company.account_purchase_tax_id:
            self.env['account.account'].browse(acc_template_ref.values()).filtered(lambda x: x.user_type_id.internal_group == 'expense').tax_ids = company.account_purchase_tax_id
        return {}
