...
class AccountChartTemplate(models.Model):
    ...
    def _prepare_all_journals(self, acc_template_ref, company, journals_dict=None):
        ...
        data to create it properly. We use a extra key in the context in order
        to properly create the sale journals for demo instances.
        """
        ...
        if company.country_id == self.env.ref('base.ar'):
            for vals in res:
                if vals['type'] == 'sale':
                    if self._context.get('create_sale_journal'):
                        vals.update({
